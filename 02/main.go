...
func read() error {
	for _, password := range raw {
		...
		p := &Password{
			min:     min,
			max:     max,
			needed:  string(parts[1][0]),
			content: string(parts[len(parts)-1]),
		}
...
func main() {
	...
	counted := make(chan bool)
	positionals := make(chan bool)
	var wg sync.WaitGroup

	for _, password := range passwords {
		wg.Add(1)
		go validatePasswordCount(password, &wg, counted)
		wg.Add(1)
		go validatePasswordPositions(password, &wg, positionals)
	}

	counts := 0
	positioned := 0
	go func() {
		for answer := range counted {
			if answer == true {
				counts += 1
			}
		}
	...
	fmt.Printf("Valid passwords with count: %v, Valid passwords with positions: %v", counts, positioned)
}

func validatePasswordCount(password *Password, wg *sync.WaitGroup, c chan bool) {
	defer wg.Done()

	count := strings.Count(password.content, password.needed)
...
func validatePasswordPositions(password *Password, wg *sync.WaitGroup, c chan bool) {
	defer wg.Done()
	if (string(password.content[password.min-1]) == password.needed) != (string(password.content[password.max-1]) == password.needed) {
		c <- true
	}
